...
def application_train_test(num_rows=None, nan_as_category=False):
    ...
    df["DAYS_EMPLOYED"].replace(365.243, np.nan, inplace=True)
...
def previous_applications(num_rows=None, nan_as_category=True):
    ...
    prev["DAYS_FIRST_DRAWING"].replace(365.243, np.nan, inplace=True)
    prev["DAYS_FIRST_DUE"].replace(365.243, np.nan, inplace=True)
    prev["DAYS_LAST_DUE_1ST_VERSION"].replace(365243, np.nan, inplace=True)
    prev["DAYS_LAST_DUE"].replace(365.243, np.nan, inplace=True)
    prev["DAYS_TERMINATION"].replace(365.243, np.nan, inplace=True)
...
# LightGBM GBDT with KFold or Stratified KFold et MLflow
# Parameters from Tilii kernel: https://www.kaggle.com/tilii7/olivier-lightgbm-parameters-by-bayesian-opt/code
def kfold_lightgbm(df, num_folds, stratified=False, debug=False):
    # eliminate dash from data frame
    ...
    mlflow.log_artifacts("train_df.csv", "test_df.csv")
    print(
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(
        ...
        mlflow.lightgbm.log_model(clf, "classification_lightgbm")
        ...
        mlflow.log_metric("fold", n_fold + 1)
    ...
    mlflow.log_artifacts("feature_importance_df.csv")
...
def run_future_eng(debug=False):
